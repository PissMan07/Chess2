import {matrix1, greenSquares} from './buildBoard.js';
matrix1.reverse();
const nPos = []
...
function pawn(x, start, end){
    if (clickedPiece !== x.id){
        ...
        if (yPos == start){
            if (matrix1[yPos+2]){ 
                nPos.push(matrix1[yPos+1][xPos],matrix1[yPos+2][xPos])
            }
            if (matrix1[yPos+1][xPos+1]){
        ...
        for (let i = 0;i<nPos.length;i++){
            const x = document.getElementById(nPos[i])
            if (x.id == matrix1[yPos+1][xPos-1]){ //check left
            ...
            if (x.id == matrix1[yPos+1][xPos+1]){ //check right
                if (x.childElementCount > 1){ //check if enemy
                    if (clickedPiece[0] == x.children[1].id[0]){
                        d.push(nPos[i])
                    }
            ...
            if (x.id == matrix1[yPos+1][xPos]){ //check up1
                if (x.childElementCount > 1){
                    d.push(nPos[i])
                }
            }
            if (yPos == start){
                if (x.id == matrix1[yPos+2][xPos]){ //check up2
                    if (x.childElementCount > 1){
                        d.push(nPos[i])
                    }
                }
...
for (let i = 0;i<pieces.length;i++){
    ...
    if (p.test(pieces[i].id)){
        if (regColor.test(pieces[i].id)){
            if (color == 'White'){
                pieces[i].addEventListener('click',function(){pawn(pieces[i],1,6)});
            } else {
                pieces[i].addEventListener('click',function(){pawn(pieces[i],6,1)});
            }
        };
    };
    //Bishop
